...
uint64_t compute_fee(uint64_t amount_to_forward, struct policy policy,uint64_t hops) {
  uint64_t fee;
  fee = (policy.fee_proportional*amount_to_forward + hops*policy.TVF) / 1000000 ;
...
unsigned int check_balance_and_policy(struct edge* edge, struct edge* prev_edge, struct route_hop* prev_hop, struct route_hop* next_hop) {
  ...
  expected_fee = compute_fee(next_hop->amount_to_forward, edge->policy,next_hop->hops_to_target);    // edited .. .
  if(prev_hop->amount_to_forward >= next_hop->amount_to_forward + expected_fee && 0 ){
...
void forward_payment(struct event *event, struct simulation* simulation, struct network* network) {
  ...
  //count_assigned[event->node_id]++;
  
  if(!is_present(next_route_hop->edge_id, node->open_edges)) {
    faulty_nodes1[event->node_id]++;
    //nodes_default_count[event->node_id]++;
